...
루트 `AGENTS.md`는 프로젝트 범위, 산출물 경계, 검증 원칙을 관리합니다. 문서 작성, 영상 제작, AI 스킬 보관처럼 모듈별 실행 방식은 각 모듈의 `AGENTS.md`에 둡니다.
...
├── 발표자료.pptx                     # Stology 발표용 슬라이드
├── Xenon_나르_김준호_Stology.pdf     # 발표/공유용 PDF
├── docs/                             # 제품 기획 및 화면 요구사항 문서
│   ├── AGENTS.md                     # 문서 작업 전용 지침
│   ├── 기획안.md                     # 서비스 배경, 목표, MVP 범위
│   ├── 화면설계서.md                 # PM 관점 화면 정보/와이어프레임/디스크립션 산출물
│   ├── 온톨로지_정의.md              # 서비스 제공 온톨로지의 제품 정의
│   ├── spring_boot_ontology_template.json # Spring Boot 온톨로지 템플릿 JSON
│   ├── 변경사항/합의_0514.md         # 합의/변경 이력
│   ├── 변경사항/합의_0612.md         # Notion/Figma/docs 기능·화면 ID 통합 합의
│   ├── 변경사항/합의_0615.md         # 지식 구조 노드 상세 패널 관계 표시 방식 합의
│   ├── 변경사항/합의_0618.md         # 지식 구조 탐색 정책 구체화 합의
│   ├── 변경사항/합의_0618_화면계약.md # Notion/Figma/docs 화면 계약 정합화 합의
│   ├── 변경사항/합의_0627_홈활동영역.md # 홈 내 할 일/팀 활동 구조 합의
│   ├── 변경사항/합의_0716_업로드주차.md # 자료 업로드 주차 선택 제거 및 자동 귀속 합의
│   ├── 변경사항/합의_0717_자료수정.md # 대기열 자료 제목·자료 설명 수정 및 검토 재수정 제거 합의
│   ├── 변경사항/합의_0717_리포트생성시간.md # 시작일 기반 주차 종료 및 리포트 생성 시간 합의
│   ├── 변경사항/합의_0717_리포트핵심노드_노드추천.md # 주차 메타데이터 기반 핵심 노드와 추천 중복 제거 합의
│   ├── 변경사항/합의_0717_팀활동행종류.md # 팀 활동 표시 행을 지식 구조 반영·다른 팀원 질문 답글로 축소한 합의
│   ├── 변경사항/합의_0716_홈활동상세모달.md # 홈 3개 상세 모달 및 팀 활동 최신순 피드 합의
│   ├── 변경사항/실행계획_0716_홈활동상세모달.md # 홈 활동 상세 모달 Notion·로컬·Figma 반영 계획
│   ├── superpowers/                   # Superpowers 기반 설계·구현 문서
│   │   ├── specs/                     # 승인된 UI/기능 설계
│   │   └── plans/                     # 구현 계획
│   └── wire_frame_requirements/      # 01~10 화면별 와이어프레임 명세
├── video/                            # HyperFrames 영상 프로젝트(로컬/ignored 산출물)
│   ├── AGENTS.md                     # 영상 작업 전용 지침
│   ├── DESIGN.md                     # 영상 디자인 소스 오브 트루스
│   ├── index.html                    # 메인 HyperFrames 컴포지션
│   ├── design-improved/              # 별도 디자인 개선판 HyperFrames 변형본
...
└── ai-skills/                        # 프로젝트 진행 중 만든 AI 스킬 보관본(로컬/ignored 산출물)
    ...
    ├── figma-production-design/      # Figma 프로덕션 디자인 제작 스킬 보관본
    └── stology-figma-ia/             # Stology Figma IA 제작/검수 스킬 보관본
```
...
2. **근거 먼저 확인**: 제품 범위, 화면 흐름, 영상 메시지는 관련 문서를 확인한 뒤 수정합니다.
...
5. **런타임/시스템 파일 제외**: `.omx/`, `.codex/`, `.gjc/`, `.superpowers/`, `.git/`, `.DS_Store`, 임시 캐시, 빌드 산출물은 프로젝트 산출물로 취급하지 않습니다. 단, 승인된 Superpowers 설계·계획은 `docs/superpowers/`에 보관합니다.
...
- PPTX/PDF 산출물을 바꾼 경우: 파일 열림 여부, 페이지/슬라이드 수, 주요 텍스트와 이미지 배치를 확인합니다.
...
- 기능·화면 ID 통합 합의: `docs/변경사항/합의_0612.md`
- 서비스 온톨로지 정의: `docs/온톨로지_정의.md`
- Spring Boot 온톨로지 템플릿: `docs/spring_boot_ontology_template.json`
- PM 화면설계서: `docs/화면설계서.md`
- 화면 목록/흐름: `docs/wire_frame_requirements/00-index.md`
- 초대 수락 화면 명세: `docs/wire_frame_requirements/10-invite-accept.md`
- 영상 디자인 기준: `video/DESIGN.md`
- 영상 구현 규칙: `video/AGENTS.md`
- 영상 메인 파일: `video/index.html`
- 별도 디자인 개선판: `video/design-improved/index.html`
- 발표 자료: `발표자료.pptx`
- 공유용 PDF: `Xenon_나르_김준호_Stology.pdf`
